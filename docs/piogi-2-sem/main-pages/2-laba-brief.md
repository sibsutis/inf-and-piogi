...
После этой лабы на экране появятся *реальные* данные с сервера. Пусть выглядят ~~стильно~~ просто, но это "Мстители", а не "Item 1".
...
==Убрать все блоки готового кода кроме самых сложных. Оставить рисунки как может быть и ссылки на примеры компонентов==
...
Создай папку (==вот тут ссылка на пространства имен==) `Models` и добавь класс `MovieDto.cs`:
...
==Думаю хватит дефолтного системного текст==
...
==Лучше добавить диаграмму класса. Файл по диаграммам классов.==
...
Создай Pull Request на GitHub и дайте мне ссылку.
Также коммиты можно разбить на более мелкие (==пример с разбиением коммиты и отдельным коммитом каждого файла в гит файле==)
